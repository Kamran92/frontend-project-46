...
const expectedPlainFormat = fs.readFileSync('__tests__/__fixtures__/mock-files/expected-plain.txt', 'utf-8')
const expectedJsonFormat = fs.readFileSync('__tests__/__fixtures__/mock-files/expected-json.txt', 'utf-8')
...
describe('format', () => {
  ...
  test('должен преобразовывать в формат "plain"', () => {
    ...
    expect(result).toBe(expectedPlainFormat)
  })

  test('должен преобразовывать в формат "json"', () => {
    const result = format(mockDiffTree, 'json')
